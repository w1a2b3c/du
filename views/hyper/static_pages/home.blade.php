...
<div class="row">
    ...
    <div class="row category">
        <div class="col-md-12">
            <h3>
                {{-- 分类名称 --}}
                <span class="badge badge-info"><i class="noti-icon uil-product-list search-icon"></i> {{ $group['gp_name'] }} </span>
            </h3>
        </div>
        <div class="col-md-12">
            <div class="card pl-1 pr-1">
                <table class="table table-centered mb-0">
                    <thead>
                        <tr>
                            {{-- 名称 --}}
                            <th width="50%">{{ __('hyper.home_product_name') }}</th>
                            {{-- 类型 --}}
                            <th width="15%" class="phone">{{ __('hyper.home_product_class') }}</th>
                            {{-- 库存 --}}
                            <th width="15%" class="phone">{{ __('hyper.home_in_stock') }}</th>
                            {{-- 价格 --}}
                            <th width="10%">{{ __('hyper.home_price') }}</th>
                            {{-- 操作 --}}
                            <th width="80px" class="text-center">{{ __('hyper.home_place_an_order') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="category">
                            ...
                            <td class="text-center">
                                @if($goods['in_stock'] > 0)
                                    {{-- 购买 --}}
                                    <a class="btn btn-outline-primary" href="{{ url("/buy/{$goods['id']}") }}"><i class="noti-icon uil-outline-buy search-icon"></i>
                                    {{ __('hyper.home_buy') }}
                                    </a>
                                @else
                                    {{-- 缺货 --}}
                                    <a class="btn btn-outline-secondary disabled" href="javascript:void(0);"><i class="noti-icon uil-outline-outs search-icon"></i>
